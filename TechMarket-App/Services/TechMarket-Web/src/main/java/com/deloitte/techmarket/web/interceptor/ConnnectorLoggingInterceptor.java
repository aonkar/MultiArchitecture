package com.deloitte.techmarket.web.interceptor;
...
public class ConnnectorLoggingInterceptor implements ClientHttpRequestInterceptor {
	...
	public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution)
		...
		log.debug("intercept Method" + getRequestDetails(request, body));
		ClientHttpResponse response = execution.execute(request, body);
		logRequestAndResposne(request, body, response);
		return response;
	}

	private StringBuilder getRequestDetails(HttpRequest request, byte[] body) {
	...
	private StringBuilder getResponseDetails(ClientHttpResponse response) throws IOException {
	...
	private void logRequestAndResposne(HttpRequest request, byte[] body, ClientHttpResponse response)
			throws IOException {
		StringBuilder sw = new StringBuilder();
		sw.append(getRequestDetails(request, body));
		sw.append(getResponseDetails(response));
		log.debug("logRequestAndResposne method", sw);
	}
